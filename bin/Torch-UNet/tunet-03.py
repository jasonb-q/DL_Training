...
class UNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.enc_channels: Tuple[int] = tuple(settings["channels"])# 1, 16, 32, 64, 128, 256 
        self.dec_channels: Tuple[int] = self.enc_channels[::-1][:-1] # 256, 128, 64, 32, 16 
...
def train(model, train_generator, val_generator, loss_fxn, optimizer, n_epochs):
    ...
    for epoch in range(n_epochs):
        ...
        mean_epoch_loss.append(np.mean(np.array(epoch_loss)))
        print(f'Epoch: {epoch}, Loss: {np.mean(np.array(epoch_loss))}')

    print(mean_epoch_loss)
    plt.plot(range(50), mean_epoch_loss)
    plt.show()
